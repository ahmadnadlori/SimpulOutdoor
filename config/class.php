...
$mysqli = new mysqli("localhost", "root", "", "simpuloutdoor");
...
class laporan
{
	...
	function kasir($bulan, $tahun)
	{
		$semuadata = array();
		$ambil = $this->koneksi->query("SELECT * FROM tbl_data_pengiriman
			WHERE id_pelanggan=0 AND status_pembelian='Selesai' AND month(tanggal_pembelian)='$bulan' AND year(tanggal_pembelian)='$tahun'");
		while ($pecah = $ambil->fetch_assoc()) {
			$semuadata[] = $pecah;
		}
		return $semuadata;
